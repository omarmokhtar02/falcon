...
class ApiFeatures {
    ...
    search(modelName) {
        if (this.queryString.search) {
            ...
            if (modelName == 'Oneway') {
                querySearch.$or = [
                    { departure_city: { $regex: this.queryString.search, $options: 'i' } },
                    { arrival_city: { $regex: this.queryString.search, $options: 'i' } },
                ]
            } else if (modelName == 'Twoway') {
                querySearch = [
                    { from_city1: { $regex: this.queryString.search, $options: 'i' } },
                    { to_city1: { $regex: this.queryString.search, $options: 'i' } },
                    { from_city2: { $regex: this.queryString.search, $options: 'i' } },
                    { to_city2: { $regex: this.queryString.search, $options: 'i' } },
                ]
            } else {
                [{ departure_city: { $regex: this.queryString.search, $options: 'i' } },
                { arrival_city: { $regex: this.queryString.search, $options: 'i' } },]
